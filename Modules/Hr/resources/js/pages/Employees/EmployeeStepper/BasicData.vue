...
const formSchema = (props: any) => ({
    formStructure: {
        ...
        gender: props.form?.gender ?? props.item?.gender ?? '',
        marital_status: props.form?.marital_status ?? props.item?.marital_status ?? '',
        phone: props.form?.phone ?? props.item?.phone ?? '',
        joining_date: props.form?.joining_date ?? props.item?.joining_date ?? '',
        national_id: props.form?.national_id ?? props.item?.national_id ?? '',
        dob: props.form?.dob ?? props.item?.dob ?? '',
        address: props.form?.address ?? props.item?.address ?? '',
...
<template>
    <div class="grid grid-cols-2 gap-3 py-4">
        ...
        <InputGroup v-model="form.name_ar" :modelValueError="props.form?.errors.name_ar" :label="$t('name_ar')"
            :placeholder="$t('please_enter_a_name_ar')" type="text" :vue-error="$v?.name_ar" />

        <InputGroup v-model="form.email" :modelValueError="props.form?.errors.email" :label="$t('email')"
            :placeholder="$t('please_enter_a_email')" type="email" :vue-error="$v?.email" />

        <InputGroup v-model="form.phone" :modelValueError="props.form?.errors.phone" :label="$t('phone')"
            :placeholder="$t('please_enter_a_phone')" type="text" :vue-error="$v?.phone" />

        <SelectGroup v-model="form.gender" :modelValueError="props.form?.errors.gender" :label="$t('gender')"
            :placeholder="$t('please_select_a_department')" :vue-error="$v?.gender" :options="props.genders || []" />

        <SelectGroup v-model="form.marital_status" :modelValueError="props.form?.errors.marital_status"
        ...
        <InputGroup v-model="form.national_id" :modelValueError="props.form?.errors.national_id"
            :label="$t('national_id')" :placeholder="$t('please_enter_a_national_id')" type="text"
            :vue-error="$v?.national_id" />

        <InputGroup v-model="form.joining_date" :modelValueError="props.form?.errors.joining_date"
            :label="$t('joining_date')" :placeholder="$t('please_enter_a_joining_date')" type="date"
            :vue-error="$v?.joining_date" />

        <InputGroup v-model="form.dob" :modelValueError="props.form?.errors.dob" :label="$t('dob')"
            :placeholder="$t('please_enter_a_dob')" type="date" :vue-error="$v?.dob" />

        <TextareaGroup v-model="form.address" :modelValueError="props.form?.errors.address" :label="$t('address')"

...
import BonusFormDialog from '../../Bonuses/BonusFormDialog.vue';
...
// reactive search
const { search } = useSearchTable(employeesActivityRoute.bonuses(props.employee.id).url, {
    bonusType: {
        name: ''
    },
    createdBy: {
...
const deleteBonuse = () => {
    ...
    router.delete(bonusesRoute.destroy(currentItem.value.id).url + `?redirect_url=${props.redirect_url}`, {
        onFinish: () => {
...
<template>
    ...
    <Can :permissions="['bonus_create', 'bonus_edit']">
        <BonusFormDialog v-model:show="showFormDialog" :method_type="method_type" :action="action" :item="currentItem"
            :loading="showLoading" :data="dataCached" :employee_id="props.employee?.id ?? null"

...
import Contract from "./ContractStepper/Contract.vue"
import Salary from "./ContractStepper/Salary.vue"
...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    { title: props.method_type === 'post' ? t('add_contract') : t('edit_contract'), href: null },
];
const stepIndex = ref(1);

const steps = ref<Array<{ step: number; title: string; status: string; form: Record<string, any> | null }>>([
    { step: 1, title: t('contract'), status: 'active', form: null },
    { step: 2, title: t('salary'), status: 'inactive', form: null },
]);
    ...
    if (stepIndex.value === steps.value.length) {
        ...
        if (canSubmit) {
            ...
            if (props.method_type === 'post') {
                form.value?.post(props.action + '?redirect_url=' + (props.redirect_url ?? ''), options);
            } else if (props.method_type === 'put') {
                form.value?.put(props.action + '?redirect_url=' + (props.redirect_url ?? ''), options);
            }
        }
    }
};
...
const stepsRef = ref([step_1, step_2]);
...
<template>
    ...
    <Head :title="props.method_type === 'post' ? $t('add_contract') : $t('edit_contract')" />

    <AppLayout :breadcrumbs="breadcrumbs">
        <Card>
            <CardHeader class="flex justify-between items-center">
                <CardTitle>
                    {{ props.method_type === 'post' ? $t('add_contract') : $t('edit_contract') }}
                </CardTitle>
            </CardHeader>
            <CardContent>
                <Stepper v-slot="{ isPrevDisabled, prevStep }" v-model="stepIndex" class="block w-full">
                    <form @submit.prevent="submitForm">
                        ...
                        <div class="flex flex-col gap-4 mt-4">
                            <Contract v-show="stepIndex === 1" :step="1" v-model:form="steps[0].form" ref="step_1"
                                :item="props.item?.contract" :divisions :departments :sections :positions
                                :work_schedules :time_managements :employees :employee_id />

                            <Salary v-show="stepIndex === 2" :step="2" v-model:form="steps[1].form" ref="step_2"

...
const action = ref(deductionTypesRoute.store().url);
...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    { title: t('deduction_types'), href: null },
];
...
/**
 * Toggle the form dialog for adding or editing a deduction_type
...
const toggleFormDialog = (item?: any) => {
    ...
    if (item) {
        ...
        action.value = deductionTypesRoute.update(item.id).url;
    } else {
        method_type.value = "post";
        action.value = deductionTypesRoute.store().url;
    }
...
// reactive search
const { search } = useSearchTable(deductionTypesRoute.index().url);

// Delete Modal
const showDeleteModal = ref(false)
const isDeleting = ref(false)

const toggleShowDeleteModal = (deduction_type: any) => {
    currentItem.value = deduction_type
...
const deleteDeductionType = () => {
    ...
    router.delete(deductionTypesRoute.destroy(currentItem.value.id).url, {
        onFinish: () => {
            showDeleteModal.value = false
            currentItem.value = null
            isDeleting.value = false
            showToast({
                title: t('deduction_type_deleted_successfully'),
                type: 'success'
            })
        },
        onError: () => {
            isDeleting.value = false
            showToast({
                title: t('deduction_type_deleted_failed'),
                type: 'error'
            })
        }
    })
...
const toggleShowDialog = (deduction_type: any) => {
    currentItem.value = deduction_type
...
const toggleImportialog = () => {
    ...
    action.value = deductionTypesRoute.import().url
    method_type.value = "post"
}
</script>
...
<template>
    ...
    <Head :title="$t('deduction_types')" />

    <AppLayout :breadcrumbs="breadcrumbs">
        <Card>
            <CardHeader class="flex justify-between items-center">
                <CardTitle>{{ $t('deduction_types') }}</CardTitle>
                <Can permissions="deduction_type_create">
                    <DropdownMenu>
                        <DropdownMenuTrigger>
                            <Button
                        ...
                        <DropdownMenuContent>
                            <DropdownMenuItem>
                                <button v-on:click="toggleFormDialog(null)" class="cursor-pointer">
                                    <i class="ri ri-add-line"></i> {{ $t("add_deduction_type") }}
                                </button>
                            </DropdownMenuItem>
                            <DropdownMenuSeparator />
                            <DropdownMenuItem>
                                <a :href="deductionTypesRoute.downloadTemplate().url" rel="noopener"
                            ...
                            <DropdownMenuItem>
                                <button v-on:click="toggleImportialog()" class="cursor-pointer">
                                    <i class="ri ri-file-excel-2-line"></i> {{ $t("import_deduction_types") }}
                                </button>
                            </DropdownMenuItem>
                        </DropdownMenuContent>
                    </DropdownMenu>
            ...
            <CardContent>
                <Table>
                    <TableCaption>{{ $t('deduction_types') }}</TableCaption>
                    <TableHeader>
                        <TableRow>
                    ...
                    <TableBody>
                        <TableRow v-for="(deduction_type, index) in props.deduction_types?.data || []"
                            :key="deduction_type.id">
                            <TableCell class="font-medium text-center">
                                <TablePaginationNumbers :items="props.deduction_types" :index="index" />
                            </TableCell>
                            <TableCell class="text-center">{{ strLimit(deduction_type.name, 15) }}</TableCell>
                            <TableCell>
                                <TableActionsDialog class="text-center flex justify-center"
                                    canShow="deduction_type_show" :show="() => toggleShowDialog(deduction_type)"
                                    canEdit="deduction_type_edit" :edit="() => toggleFormDialog(deduction_type)"
                                    canDelete="deduction_type_delete"
                                    :delete="() => toggleShowDeleteModal(deduction_type)" />
                            </TableCell>
                        </TableRow>
                    </TableBody>

                    <TableFooter>
                        <TableEmpty v-if="!props.deduction_types?.data?.length" :colspan="8">
                            {{ $t('no_data') }}
                        </TableEmpty>
                    </TableFooter>
                </Table>
            ...
            <CardFooter>
                <PaginationUse :items="props.deduction_types?.links || []" :total="props.deduction_types?.total || 0"
                    :itemsPerPage="props.deduction_types?.per_page || 10"
                    :currentPage="props.deduction_types?.current_page || 1" :defaultPage="1" />
            </CardFooter>
        </Card>
    </AppLayout>

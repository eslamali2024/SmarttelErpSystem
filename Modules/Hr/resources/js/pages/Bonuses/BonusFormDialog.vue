...
<template>
    <Dialog :open="show" @update:open="val => emit('update:show', val)">
        <DialogContent class="sm:max-w-[425px] md:max-w-[600px] lg:max-w-5xl">
            <DialogHeader>
                <DialogTitle>
                    {{ props.method_type === 'post' ? $t('add_bonus') : $t('update_bonus') }}
                </DialogTitle>
            </DialogHeader>
            <DialogDescription :loading="loading">
                <div class="grid grid-cols-12 gap-3 py-4">
                    ...
                    <SelectGroup v-model="form.bonus_type_id" :modelValueError="form.errors.bonus_type_id"
                        ...
                        :options="props.data?.bonus_types" :placeholder="$t('please_select_a_bonus_type')" type="text"
                        :vue-error="$v?.bonus_type_id" />
